# Description: Tests for the OTLP parser module
# Description: Validates parsing of resources, scopes, metric definitions, and time-series points
...
def test_parse_data_point_with_int():
    ...
    result = parse_data_point(data_point, "test_hash_456", "Memory", None, "memory.bytes")
...
def test_parse_metric_gauge():
    ...
    metric_def, data_points = parse_metric(metric, "resource_hash_123", "CPU_Usage", "1.0")
...
def test_parse_metric_sum():
    ...
    metric_def, data_points = parse_metric(metric, "resource_hash_456", "HTTP_Requests", "2.0")
        ...
        "metrics": [
            {
                "name": "cpu.usage",
                "unit": "percent",
                "gauge": {"dataPoints": [{"timeUnixNano": 1699123456000000000, "asDouble": 45.2}]},
            }
        ],
    }
                ...
                "metrics": [
                    {
                        ...
                        "unit": "percent",
                        "gauge": {
                            "dataPoints": [{"timeUnixNano": 1699123456000000000, "asDouble": 45.2}]
                        },
                    }
                ],
            }
...
def test_multiple_scope_metrics_per_resource():
    ...
    resource_metrics = {
        "resource": {"attributes": [{"key": "host", "value": {"stringValue": "server01"}}]},
        "scopeMetrics": [
            {
                ...
                "metrics": [
                    {
                        "name": "memory.usage",
                        "gauge": {
                            "dataPoints": [{"timeUnixNano": 1699123456000000000, "asInt": 1024}]
                        },
                    }
                ],
            },
