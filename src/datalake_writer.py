...
def _sanitize_float(value: float | None) -> float | None:
    ...
    Many downstream SQL engines reject IEEE-754 special values in
    queries, so we normalize them to NULL at write time.
    """
    if value is None:
        return None
...
class DataLakeConfig:
    ...
    def from_env(cls) -> "DataLakeConfig":
        ...
        Raises ValueError if DATALAKE_ACCOUNT is not set so misconfigurations
        fail loudly instead of writing to a wrong account.
        """
        account = os.getenv("DATALAKE_ACCOUNT", "")
        if not account:
            raise ValueError(
                "DATALAKE_ACCOUNT environment variable is required "
                "(set it to the Azure Data Lake Gen2 storage account name)"
            )
        return cls(
            account_name=account,
            filesystem=os.getenv("DATALAKE_FILESYSTEM", "metrics"),
            base_path=os.getenv("DATALAKE_BASE_PATH", "otlp"),
            flush_interval_seconds=int(os.getenv("DATALAKE_FLUSH_INTERVAL_SECONDS", "600")),
            flush_threshold_rows=int(os.getenv("DATALAKE_FLUSH_THRESHOLD_ROWS", "50000")),
        )
...
class DataLakeWriter:
    ...
    async def _flush_metric_data(self, fs_client) -> int:
        ...
        for row in self.metric_buffer:
            ...
            if isinstance(ts, datetime):
                ...
                dt = ts if isinstance(ts, datetime) else datetime.fromtimestamp(ts / 1e6, tz=UTC)
        ...
        for partition_path, rows in partitions.items():
            ...
            filename = f"part-{ts}-{uuid.uuid4().hex[:8]}.parquet"
            full_path = f"{self.config.base_path}/metric_data/{partition_path}/{filename}"

            # Upload to Data Lake
            file_client = fs_client.get_file_client(full_path)
            await asyncio.to_thread(file_client.upload_data, buffer.getvalue(), overwrite=True)
    ...
    async def _flush_resources(self, fs_client) -> None:
        ...
        ts = datetime.now(UTC).strftime("%Y%m%d%H%M%S")
        filename = f"resources-{ts}-{uuid.uuid4().hex[:8]}.parquet"
    ...
    async def _flush_metric_definitions(self, fs_client) -> None:
        ...
        ts = datetime.now(UTC).strftime("%Y%m%d%H%M%S")
        filename = f"metric_definitions-{ts}-{uuid.uuid4().hex[:8]}.parquet"
    ...
    def _datapoint_to_dict(self, dp: MetricDataPoint, ingested_at: datetime) -> dict[str, Any]:
        ...
        Sanitizes NaN/Infinity values to NULL for downstream SQL engines.
        """
        ts = dp.timestamp
        return {
