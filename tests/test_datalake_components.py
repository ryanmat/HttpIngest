...
class TestDataLakeConfig:
    ...
    def test_from_env_requires_account(self):
        """DataLakeConfig.from_env() raises when DATALAKE_ACCOUNT is missing."""
        with (
            patch.dict(os.environ, {}, clear=True),
            pytest.raises(ValueError, match="DATALAKE_ACCOUNT"),
        ):
            DataLakeConfig.from_env()

    def test_from_env_defaults(self):
        """DataLakeConfig uses sensible defaults when only the account is set."""
        env = {"DATALAKE_ACCOUNT": "myaccount"}
    ...
    def test_from_env_custom(self):
        """DataLakeConfig honors custom env vars."""
        custom_env = {
...
class TestIngestionRouter:
    ...
    async def test_ingest_handles_datalake_error(self, router, mock_datalake_writer):
        ...
        mock_datalake_writer.write_metrics = AsyncMock(side_effect=Exception("Upload failed"))
    ...
    async def test_get_status(self, router, mock_datalake_writer):
        ...
        mock_datalake_writer.get_buffer_stats = MagicMock(return_value={"metric_data_buffered": 5})
