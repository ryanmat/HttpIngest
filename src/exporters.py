...
@dataclass
class AlertEvent:
    """Alert event for webhook notifications."""
    alert_id: str
...
class WebhookNotifier:
    ...
    Sends alert events to external systems via HTTP webhooks.
    Supports HMAC signatures for authentication.
    """
    ...
    def send_alert(
        ...
        if webhook_config.secret:
            signature = self._compute_hmac(payload, webhook_config.secret)
    ...
    def _prepare_payload(self, alert: AlertEvent) -> str:
