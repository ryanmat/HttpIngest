...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--storage-account",
        default=os.environ.get("DATALAKE_ACCOUNT", ""),
        help="ADLS storage account (required; reads DATALAKE_ACCOUNT env var as default)",
    )
    parser.add_argument(
        "--container",
        default=os.environ.get("DATALAKE_FILESYSTEM", DEFAULT_CONTAINER),
...
def compact_partition(
    ...
    logger.info(f"  {day_path}: {file_count} files, {source_count} rows")
    ...
    with tempfile.NamedTemporaryFile(suffix=".parquet", delete=True) as tmp:
        ...
        verify_count = conn.execute(f"SELECT COUNT(*) FROM read_parquet('{tmp_path}')").fetchone()[
            0
        ]
...
def main() -> int:
    ...
    if not args.storage_account:
        logger.error("Missing storage account: pass --storage-account or set DATALAKE_ACCOUNT")
        return 1
